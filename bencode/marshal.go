	...
	case BLIST:
		for i, o := range list {
			...
			if v.Type().Elem().Kind() != reflect.Slice {
				return errors.New("type error: expected reflect.Slice")
			}
	...
	case BDICT:
		for i, o := range list {
			...
			if v.Type().Elem().Kind() != reflect.Struct {
				return errors.New("type error: expected reflect.Struct")
			}
...
func marshalValue(w io.Writer, v reflect.Value) (int, error) {
	...
	case reflect.Struct:
		len += marshalDict(w, v)
	default:
		return -1, errors.New("unsupport type")
	}
	return len, nil
}
...
func Marshal(w io.Writer, s interface{}) (int, error) {
	v := reflect.ValueOf(s)
